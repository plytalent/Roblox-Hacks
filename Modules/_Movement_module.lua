return function (Variables, Tabs, ManaFlyToggleOption, ManaRunToggleOption, MovementSpeedOption)
	...
	local ManaFlyToggle = Tabs[1]:AddToggle("_Fly", _ManaFlyToggleOption)
	local ManaRunToggle = Tabs[1]:AddToggle("_Speed", _ManaRunToggleOption)
	...
	local ManaSpeed = Tabs[1]:AddSlider("_MovementSpeed", _MovementSpeedOption)

	local MovementModeDropdown = Tabs[1]:AddDropdown("_MovementMode",{
		Title = fly_name.."/"..run_name.." Type",
		Values = {"Positioning", "Velocity", "BodyMover(Velocity)", "BodyMover(Position)", "AlignPosition"},
		Default = "Positioning",
	})
	local ManaFlyKey = Tabs[2]:AddKeybind("_FlyKeyBind", {
		Title = fly_name.." Key",
		Mode = "Toggle",
		Default = "K",
	})
	local ManaRunKey = Tabs[2]:AddKeybind("_RunKeyBind", {
		Title = run_name.." Key",
		Mode = "Toggle",
		Default = "M",
	})
