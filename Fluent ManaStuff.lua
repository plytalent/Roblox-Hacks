...
RunService.Stepped:Connect(function()
    ...
    if (Options.Fly.Value or Options.Speed.Value) and env.MovementMode == "BodyMover(Velocity)" then
        env.BodyVelocity.Parent = rootpart
    else
        env.BodyVelocity.Parent = nil
    end
    if (Options.Fly.Value or Options.Speed.Value) and env.MovementMode == "BodyMover(Position)" then
        env.BodyPosition.Parent = rootpart
    else
        env.BodyPosition.Parent = nil
    end
    if (Options.Fly.Value or Options.Speed.Value) and env.MovementMode == "AlignPosition" then
        ...
        if Options.Fly.Value or Options.Speed.Value then
            ...
            if Character ~= nil then
                ...
                elseif env.MovementMode == "BodyMover(Velocity)" then
                    cf_of_hrp = Character:GetPrimaryPartCFrame()
                    local change = (Options.MovementSpeed.Value/1.5)
                    if Options.Fly.Value then
