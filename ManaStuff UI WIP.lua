if not _G.Loading then
    _G.Loading = true
else
    return
end
if _G.RUBY_LOADED then
    return
end
            ...
            function TabInit:CreateSection(Name)
                ...
                function SectionInit:CreateDropdown(Name, OptionTable, Callback, InitialValue)
                    ...
                    function DropdownInit:SetOption(Name)
                        local Options = Dropdown.Container.Holder.Container:GetChildren()
...
function placeid_check()
    ...
    return places[tostring(game.PlaceId)]
end
local game_name = placeid_check()
if game_name == "Critical Legends" then
    Config.WindowName = "RubyTheSilent Hub [Critical Legends]"
elseif game_name == "Deepwoken" then
    Config.WindowName = "RubyTheSilent Hub [Deepwoken]"
elseif game_name == "PM" then
    Config.WindowName = "RubyTheSilent Hub [PM]"
end
...
local Artificial_Event_Module = function()
    ...
    coroutine.wrap(function()
        while true do
            ...
            tf = tf + delta
            if tf >= (current_target_frame*(ArtificialEV.Frame_Percent_Tile/100)) then
...
--local consoles_settings_tab = Window:CreateTab("Consoles Settings")
--local consoles_management = consoles_settings_tab:CreateSection("Console Management")
Artificial:Wait()
...
local ManaFly_Facing_Cam = movement:CreateToggle(Label_config.ManaFly .. "Facing Cam", false, function()end)
...
local Options_FlyMode = player:CreateDropdown("Fly Mode", {"Positioning", "Velocity"}, function(Name)if Playerobj.Character then cf_character_update=Playerobj.Character:GetPrimaryPartCFrame()end end)
Options_FlyMode:SetOption("Positioning")
--Options_RunMode:SetOption("Positioning")
...
local Frame_Tile = Event:CreateSlider("Event Frame Tile",1,100,100,true, function(Value)
    Artificial.Frame_Percent_Tile = Value
end)
...
Options_Events:SetOption("Stepped")
...
local Slider6 = UI_2:CreateSlider("Transparency",0,1,0,false, function(Value)
...
local last_tick_stepped = tick()

game:GetService("RunService").Stepped:Connect(function()
    local start = tick()
    local delta = start - last_tick_stepped
    last_tick_stepped = start
    local flymode = Options_FlyMode:GetOption()
    local Character = Playerobj.Character
    ...
    if ManaFly:GetState() and flymode == "Velocity" then
        if Character ~= nil then
            ...
            if rootpart then
                ...
                local newvelocity = (CFrame.new(rootpart.Position, cf_character_update.Position).LookVector * (variables.manaflyspeed*10)) + Vector3.new(0, 9.8, 0)
                rootpart.Velocity = newvelocity * (cf_character_update.Position - (rootpart.Position + (newvelocity * delta))).Magnitude
            end
        end
    end
    watch["ManaFly2"] = tick() - start
end)
Artificial:Bind("Fly",function(d)
    local start_overall = tick()

    local flymode = Options_FlyMode:GetOption()
    --local runmode = Options_RunMode:GetOption()
            ...
            if rootpart then
                if flymode == "Positioning" then
                    ...
                    if Character.PrimaryPart then
                        ...
                        local new_cf = ManaFly_Facing_Cam:GetState() and cf_character_update or CFrame.new(cf_character_update.Position) * CFrame.Angles(rz_ch, rx_ch, ry_ch)
                        Character:SetPrimaryPartCFrame(new_cf)
    ...
    local start2 = tick()
    if ManaRun:GetState()--[[ and runmode == "Positioning"]] then
        if Character ~= nil then
            local leftorright,backorforward = 0,0
            if UIS:IsKeyDown(Enum.KeyCode.S) then
...
function loadmodules()
    ...
    elseif game_name == "PM" then
        local s, e = pcall(function()
            local PM_Tab = Window:CreateTab("PM")
            local Teleport_Place_Section = PM_Tab:CreateSection("Places")
            local Dropdown = Teleport_Place_Section:CreateDropdown("Targets", {"Main Menu", "Hueco Mundo", "Karakua Town", "Ranked", "Soul Society", "Raids Lobby"}, function(Name)
                local teleports_ids = {
                    ["Main Menu"] = 9447079542,
                    ["Hueco Mundo"] = 10202329527,
                    ["Karakua Town"] = 9474703390,
                    ["Ranked"] = 9952723123,
                    ["Soul Society"] = 11420877323,
                    ["Raids Lobby"] = 11675417468
                }
                game:GetService("TeleportService"):Teleport(teleports_ids[Name], Playerobj)
            end)
        end)
        if not s then
    ...
    elseif game_name == "Deepwoken" and Drawing then
        local s, e = pcall(function()
            ...
            local Remote_HiJack_Section = Deepwoken_Tab:CreateSection("Remote Hi-jack")
            if typeof(hookmetamethod) == "function" and typeof(newcclosure) == "function" and typeof(checkcaller) == "function" then
                local NoFall_Toggle = Remote_HiJack_Section:CreateToggle("No Fall", true, function()end)
                local MomentumBoost_Toggle = Remote_HiJack_Section:CreateToggle("Momentum Boost", true, function()end)
                local Momentumboost_Slider = Remote_HiJack_Section:CreateSlider("Momentum Boost %", 1, 200, variables.boost or 10, true, function(x)
                    variables.boost = x
                end)
                local original
                original = hookmetamethod(game, "__namecall", newcclosure(function(remote, ...)
                    if typeof(remote) == "Instance" and not checkcaller() then
                        ...
                        if validInstance and (methodName == "FireServer" or methodName == "fireServer") then
                            if remoteName == "ServerMomentum" and typeof(args[1]) == "number" and MomentumBoost_Toggle:GetState() then
                                if args[1] > 0 then
                                    local newServerMomentum = args[1]*(1+(variables.boost/100))
                                    if newServerMomentum > 10 then
                                        newServerMomentum = 10
                                    end
                                    return original(remote, args[1]*2)
                                end
                            elseif #args == 2 and typeof(args[1]) == "number" and typeof(args[2]) == "boolean" and NoFall_Toggle:GetState() then
